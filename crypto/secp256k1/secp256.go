...
import (
	...
	"github.com/zhi-ming-zhang/go-vanity/crypto/entropy"
)
...
func GenerateKeyPair() ([]byte, []byte) {
	var seckey []byte = entropy.GetEntropyCSPRNG(32)
...
func Sign(msg []byte, seckey []byte) ([]byte, error) {
	...
	nonce := entropy.GetEntropyCSPRNG(32)
...
func SeckeyToPubkey(seckey []byte) []byte {
	var seckey_ptr *C.uchar = (*C.uchar)(unsafe.Pointer(&seckey[0]))
	var pubkey64 []byte = make([]byte, 64) // secp256k1_pubkey
	var pubkey65 []byte = make([]byte, 65) // 65 byte uncompressed pubkey
	pubkey64_ptr := (*C.secp256k1_pubkey)(unsafe.Pointer(&pubkey64[0]))
